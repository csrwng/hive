...
// +kubebuilder:rbac:groups=hive.openshift.io,resources=syncsetinstances,verbs=get;create;update;delete;patch;list;watch
func (r *ReconcileSyncSetInstance) Reconcile(request reconcile.Request) (reconcile.Result, error) {
	...
	if !controllerutils.HasFinalizer(ssi, hivev1.FinalizerSyncSetInstance) {
		ssiLog.Debugf("adding  finalizer")
		if err := r.addSyncSetInstanceFinalizer(ssi); err != nil {
			ssiLog.WithError(err).Error("error adding finalizer")
			return reconcile.Result{}, err
		}
		return reconcile.Result{}, nil
	}
	...
	spec, err := r.getSyncSetCommonSpec(ssi, ssiLog)
	if spec == nil || err != nil {
		return reconcile.Result{}, err
	}
	...
	if err != nil {
		ssiLog.WithError(err).Error("unable to load admin kubeconfig secret")
		return reconcile.Result{}, err
	}
...
func (r *ReconcileSyncSetInstance) addSyncSetInstanceFinalizer(ssi *hivev1.SyncSetInstance) error {
	controllerutils.AddFinalizer(ssi, hivev1.FinalizerSyncSetInstance)
	return r.Update(context.TODO(), ssi)
}
...
func (r *ReconcileSyncSetInstance) getSyncSetCommonSpec(ssi *hivev1.SyncSetInstance, ssiLog log.FieldLogger) (*hivev1.SyncSetCommonSpec, error) {
	if ssi.Spec.SyncSet != nil {
		...
		return &syncSet.Spec.SyncSetCommonSpec, nil
	} else {
		...
		return &selectorSyncSet.Spec.SyncSetCommonSpec, nil
	}
...
func (r *ReconcileSyncSetInstance) deleteSyncSetResources(syncSetStatus hivev1.SyncSetObjectStatus, dynamicClient dynamic.Interface, ssiLog log.FieldLogger) (hivev1.SyncSetObjectStatus, error) {
